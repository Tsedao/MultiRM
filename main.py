...
if __name__ == "__main__":
    ...
    if args.save:
        ...
        pd.DataFrame(data=labels,index=RMs,columns=cols_name_2).to_csv(os.path.join(args.save_path,args.save_id,'pred_labels.csv'),header=True)
        pd.DataFrame(data=attention,index=RMs,columns=cols_name_2).to_csv(os.path.join(args.save_path,args.save_id,'attention.csv'),header=True)
